    ...
    bool wordBreak(string s, vector<string>& wordDict) {
        ...
        for(int i = 1; i <= strsize; i++){
            for(const auto& word : wordDict){
                //if word size > i, then the word is not a substring
                //of our string, so skip this word
                if(i < word.size())
                    continue;
                /* string[1...i] = prefix[1...mididx-1]+suffix[mididx...]
                 * ex: string = "hello", word = "llo".
                 *    when i = 5, mididx = 5-3=2
                 *    "hello" = "he" + "llo" -->2. we look up the dictionary to 
                 *                                 see if "llo" is one of a word in it.
                 *                |----------> 1. we access prefixInDict[2] to check
                 *                                that "he" can be decomposed or not
                 *    if 1,2 are true, then string "hello" can be decomposed,
                 *    so we set prefixInDict[5] = true
                 */
