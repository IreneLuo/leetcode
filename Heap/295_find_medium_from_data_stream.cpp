 ...
 * 2. if the value is smaller than the min(upperHalf)=upperHalf.top(),
 *    push it into the lowerHalf, otherwise push it into the upperHalf
 ...
 *   5.  7,8                9,10         // 7 < 9 push to lowerHalf 
 *                                          --> at this moment, the median = (8 + 9) / 2
 *   6. 6,7,8               9,10         // 6 < 9 push to lowerHalf
 *   7.  6,7                8,9,10       // balance both heaps
 *                                          --> at this moment, the median = 8
 *
 *   time complexity: 3n * log(n/2) = O(nlogn)
 *     - insert: log(n/2) for n times
 *       (each heap will have maximum n/2 items, so insert an item needs log(n/2))
 *     - balanced(push/pop): 2*log(n/2) for n times
 *       (push action needs to pop-up the value to the root
 *        --> log(n/2), pop action needs to pop-down-->log(n/2))
 *     - get the answer: O(1)
