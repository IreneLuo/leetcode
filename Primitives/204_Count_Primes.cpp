 ...
 */
	if (n <=1) return 0;
        ...
        ans[1] = false;
        for (int i = 1; (i*i) < n; i++){
            /* Assume that i = 1 to n
             * if ans[i] is a prime, then change its multiples to false
             * The multiples look like this:
             *     2,4,6,8,...n: n/2 times
             *     3,6,9,12,...n: n/3 times
             *     k : n/k times(k is the biggest prime smaller than n)
             *                  (assume that k==n)
             * ==> n/2+n/3+n/5+...+n/n = n(1/2+1/3+..1/n) = n(loglogn)
             * However, our i starts from 1 to sqrt(n) for speed up
             * so time = sqrt(n)loglog(n)
             */
            if (ans[i]){
